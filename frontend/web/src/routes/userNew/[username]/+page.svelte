<script>
    ...
    onMount(async () => {
        try {
            ...
            noAboutMe = $user.aboutme ? false : true            
            noSocials = $user.socials.length === 0
            noInterests = $user.interests.length === 0
    ...
    let instagram = $user.socials.find(social => social.platform === "instagram");
    let linkedin = $user.socials.find(social => social.platform === "linkedIn");
    let facebook = $user.socials.find(social => social.platform === "facebook");
    let github = $user.socials.find(social => social.platform === "github");
    ...
    <div id="bigcont" class="flex flex-col items-center w-full h-screen my-8 gap-2 max-w-[1300px] max-2xl:w-[90%]">
        ...
        <div class=" bg-zinc-900 flex self-stretch flex-row gap-[10px] justify-start border-2 h-fit md:min-h-[800px] max-md:flex-col" id="outer_box">
            ...
            <div class="flex flex-col border-2 w-1/3 max-md:w-full bg-zinc-950 p-[10px] relative" id="inner_box_right">
                <RightBoxContent loggedinFlag={loggedinFlag} noAboutMe={noAboutMe} noSocials={noSocials} noInterests={noInterests}
                aboutMe={aboutMe} linkedin={linkedin} instagram={instagram} facebook={facebook} github={github} class="relative">
                </RightBoxContent>
            </div>
        </div>
    </div>
